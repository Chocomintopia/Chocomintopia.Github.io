...
但其实我们想做的是攻击连招 这个随机切换动作更适合多个idle动作 但是idle动作不需要按键触发 所以不适合写成动画蒙太奇并在C++中写播放函数 直接在动画蓝图状态机里做就可以 打开Main States动画蓝图 进入Ground Locomotion的Idle状态 现在我们是用一个blend poses ECharacterState节点连接了不同状态下的不同idle动作
我们想要做的是 如果角色站在原地不动超过5s 就开始随机播放多个idle动画 如果不超过5s就是默认的基础idle动画

应该专门为随机播放idle设置一个状态 命名为RandomIdle 在Ground Locomotion状态机中 和Idle Run状态形成三角 并设置状态转换规则 Idle到RandomIdle是双向箭头 RandomIdle到Run是单向箭头
```c++
// HelloWorldCharacter.h
...
Idle到RandomIdle的转换规则 拖入Character State变量 引脚往右拖搜索== 选择equal enum节点 下拉菜单选Unequipped 再把IdleTimer变量拖入 引脚往右拖搜索> 选择Greater 数字填入5 从这个Greater节点的引脚往右拖 搜素and 选择and boolean 把equal enum那个节点也连接到AND节点上 AND节点返回到Result节点
RandomIdle到Idle的转换规则 右键搜索get relevant anim time(randomidle) 意思就是动画剩余时间 再从右侧引脚往右拖 搜索== 再连接到result节点
RandomIdle到Run的转换规则 拖入GroundSpeed变量 再从引脚往右拖搜索> 再将右侧引脚连接到Result节点
